    ...
    public function groupByData(Request $request)
    {
        ...
        $transactionAggregatesQuery = (clone $baseQuery)
            ->selectRaw(
                ...
                SUM(CASE WHEN payment_status = "paid" AND payment_method = "cash" THEN total_price ELSE 0 END) as sum_cash_payment,
                SUM(CASE WHEN payment_status = "paid" AND payment_method = "bank_transfer" THEN total_price ELSE 0 END) as sum_bank_transfer_payment,
                SUM(CASE WHEN payment_status = "paid" AND payment_method = "ewallet" THEN total_price ELSE 0 END) as sum_ewallet_payment,
                SUM(CASE WHEN payment_status = "paid" AND payment_method = "qris" THEN total_price ELSE 0 END) as sum_qris_payment,
